...
for X in 1 2 3  
do
    for PAR in  32 64 #4 8 16 32 64 #128 #4 #16 32 64 #128 256 #512
    do
        case $PAR in
                8) 
                    PARALLELISM1=4
        ...
        do
            ...
            DATE=$(date +"%Y-%m-%d_%T")
            WORKSPACE=$CCCSCRATCHDIR/DASK_IPCA_WCONF_SAME_CHUNK/E${X}/P${PAR}_W${NWORKER}_D${DATA}_D${DATE}/
            mkdir  -p $WORKSPACE
	    echo $WORKSPACE
            cd $WORKSPACE
            ...
            ccc_msub -m work,scratch -r simu_post -q skylake -Q normal  -A gen2224  -x  -N $NNODESSIMU -n $NPROCSIMU   Script.sh
