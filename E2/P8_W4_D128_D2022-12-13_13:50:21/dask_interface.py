...
def connect(sched_file):
    ...
    adr = s["address"]
    try:
        client  = Client(adr, timeout=3600, heartbeat_interval=120)
...
class Bridge:
    ...
    def publish_data(self, data, data_name, timestep):
        ...
        if publish:
            #print(data_name, "is shared from process", self.position, " in timestep", timestep, flush=True)
            ...
            ts = time.time() - ts
            print("scatter   : ", ts,"secondes" , flush=True )
            data=None
        else:
...
class Adaptor:
    ...
    def __init__(self, Sworker, scheduler_info):
        with open(scheduler_info) as f:
            s = json.load(f)
        self.adr = s["address"]
        #self.client  = Client(self.adr) #, serializers=['dask', 'pickle'])
        try:
            self.client  = Client(self.adr, timeout=3600)
        except Exception as e:
            print("retrying ...", flush=True)
            self.client = Client(self.adr, timeout=3600)
        ...
        dask.config.set({"distributed.deploy.lost-worker-timeout": 120, "distributed.workers.memory.spill":0.97, "distributed.workers.memory.target":0.95, "distributed.workers.memory.terminate":0.99 })
        self.workers =  list(self.client.scheduler_info()["workers"].keys()) #[comm.get_address_host_port(i,strict=False) for i in self.client.scheduler_info()["workers"].keys()]
