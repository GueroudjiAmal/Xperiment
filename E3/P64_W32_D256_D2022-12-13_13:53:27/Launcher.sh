...
for X in 1 2 3  
do
    for PAR in  4 8 16 32 64 #4 #16 32 64 #128 256 #512
    do
        case $PAR in
                8) 
                    PARALLELISM1=4
        ...
        do
            ...
            mkdir -p $CCCSCRATCHDIR/DASK_PHD_IPCA_FIN
            DATE=$(date +"%Y-%m-%d_%T")
            WORKSPACE=$CCCSCRATCHDIR/DASK_PHD_IPCA_FIN/E${X}/P${PAR}_W${NWORKER}_D${DATA}_D${DATE}/
            mkdir  -p $WORKSPACE
            cd $WORKSPACE
