...
do
    ...
    do
        ...
        do
            ...
            #mkdir -p DEISA
            DATE=$(date +"%Y-%m-%d_%T")
            WORKSPACE=$CCCSCRATCHDIR/DEISA_V2_DERIVATIVE_WCONFIG_OMP_HEART_PERF/E${X}/P${PAR}_W${NWORKER}_D${DATA}_D${DATE}/
            mkdir  -p $WORKSPACE
            cd $WORKSPACE
            ...
            ccc_msub  -m work,scratch -r dask-cluster -q skylake -Q normal -A gen2224 -x  -N $NNODES -n $NPROC Script.sh     
        done
    done
done 
