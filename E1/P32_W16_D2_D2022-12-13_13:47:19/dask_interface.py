...
class Bridge:
    ...
    def publish_data(self, data, data_name, timestep):
        ...
        if publish:
            ...
            #tracer.runctx('f = self.client.scatter(data, direct = True, workers=self.workers, keys=[key], deisa=True)', locals={ 'self': self, 'data': data,'self.workers': self.workers, 'key': key })
